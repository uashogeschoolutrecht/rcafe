...
```
 ...
 - Age of the floppy-disk (University email)
 - Combining two computers into one
 - Building websites with MS Frontpage
 - System admin and treasurer for UBV (Bookkeeping skills)
...
## **Discuss with your neighbour** 
 ...
 - What is special about a dataframe (as a list), compared to usual lists in R?
...
## List-column characteristics
```{r}
class(table$e)
is.vector(table$e)
is.recursive(table$e)
```
...
## Iterate to transform
```{r}
random_list <- list(a = rnorm(100, mean = 10, sd = 2),
                 b = rnorm(10, mean = 5, sd = 0.5),
                 c = rnorm(1000, mean = 100, sd = 20))
log10_list <- map(random_list, log10)
map(log10_list, head, 2)
```
...
Write a piece of code that displays a graph displaying the change in pertussis cases over time for a number of countries 
...
## Plot residuals
```{r}
plot_res <- netherlands_data %>%
  na.omit() %>%
  add_residuals(netherlands_model) %>% 
...
## Split the data by country and apply the model to each subset
 ...
 - In fact, data for each country is a dataframe
...
## **Discuss with your neighbour**:
 ...
 -- AGAIN: PLEASE DO NOT SCROLL AHEAD --
...
## Better way to keep track of your results and models
 ...
 4. Apply the functions to your list column (by iteration, using the `map()` family of functions)
 ...
 7. You can even store graphs this way, you will see!

## Creating a list column with each country as an element in the list (dataframe in a cell of the nested dataframe)
We also remove the NAs.
```{r}
...
## **Discuss with you neighbour**
 ...
 - How would you label (name) the idividual elements of the list column (`$data`)?
...
```{r}
resids <- nested_pertussis$resids[[1]]
